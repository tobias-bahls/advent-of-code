...
private fun tiltStep(grid: Grid<Day14Tile>, direction: CardinalDirectionOrthogonal): Grid<Day14Tile> {
    return Grid(grid.tiles.map {
        when (it.data) {
            FLOOR -> {
                val neighbourInOppositeDirection = it.directNeighbourInDirection(direction.opposite())
                val willBeMovedOn =
                    neighbourInOppositeDirection != null && neighbourInOppositeDirection.data == ROUND_ROCK
            ...
            ROUND_ROCK -> {
                ...
                val canMove = neighbourInTiltDirection != null && neighbourInTiltDirection.data == FLOOR
...
private fun calculateLoadOnNorthSupportBeam(grid: Grid<Day14Tile>) =
    grid.tiles
        .filter { it.data == ROUND_ROCK }
        .sumOf {
            grid.height - it.point.y
        }
...
fun main() {
    ...
    part2 {
        ...
        fun oneTurn(grid: Grid<Day14Tile>): Grid<Day14Tile> {
            ...
            return tiltDirs.fold(grid) { g, dir -> g.transformUntilStable { tiltStep(it, dir) }.resultGrid }
        }
        ...
        seen[loopStart+idx].points.sumOf {
            startGrid.height - it.y
        }
    }
}