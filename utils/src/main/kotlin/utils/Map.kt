...
fun <K, V> MutableMap<K & Any, V & Any>.merge(other: Map<K & Any, V & Any>, combine: (V, V) -> V): MutableMap<K & Any, V & Any> {
    other.forEach { (k, v) -> this.merge(k, v, combine) }

    return this
}

fun <K, V> Map<K & Any, V & Any>.merge(other: Map<K & Any, V & Any>, combine: (V, V) -> V): Map<K & Any, V & Any> =
